	...
	aj.processHandle = function() {
		if(aj.XMLHttpRequest.readyState == 4 && aj.XMLHttpRequest.status == 200) {
			...
			if(aj.recvType == 'HTML') {
                ...
                var responseXML = parser.parseFromString(aj.XMLHttpRequest.responseText.replace(/<\?xml.*?\?>/,''),'text/xml');

                console.log(responseXML);
				if(!responseXML || !responseXML.lastChild || responseXML.lastChild.localName == 'parsererror') {
					aj.resultHandle('' , aj);
				} else {
					aj.resultHandle(responseXML.lastChild.firstChild.nodeValue, aj);
				}
