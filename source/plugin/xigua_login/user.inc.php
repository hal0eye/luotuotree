...
if(submitcheck('searchtxt')){
    ...
    if(strlen($txt)==11 && is_numeric($txt)){
        $user = DB::fetch_all('SELECT * FROM %t WHERE mobile IN (%n)', array('common_member_wechat', array($txt)), 'uid');
...
if(submitcheck('form')){
    foreach ($_GET['form'] as $index => $item) {
        if(trim($index) && $item){
            DB::update('common_member_wechat', array('uid' => $item), array('openid' => $index,));
        }
    }
}
...
foreach ($list as $row) {
    ...
    $rowbody = array(
        $row['openid'],
        $row['mobile'],
        "<input name='form[{$row['openid']}]' value='$uid' />",
...
showsubmit('permsubmit', 'submit', '', '', $multipage.'<input type="hidden" name="actid" id="actid" value="0" /><input type="hidden" name="acter" id="acter" value="" />');
showtablefooter();
showformfooter();
?>
<script>
