...
$reg_nickname = str_replace(array(" ", "\t", "\n", "\r"), '', $reg_nickname);
if(!$openid){
...
if($uid = $_G['uid']){
    if($reg_sid){
        ...
        dheader("Location: home.php?mod=space&uid={$uid}&do=profile&mycenter=1");
    }
}
if(!$_GET['mobile']){
    $_GET['mobile'] = getcookie('bmobile');
}

$navtitle1 = lang('plugin/xigua_login', 'newbtn');
$tip1      = lang('plugin/xigua_login', 'newtip');
$navtitle2 = lang('plugin/xigua_login', 'bind');
$tip2      = lang('plugin/xigua_login', 'bindexists');
...
if(!submitcheck('reg')){
    ...
    if($_GET['password'] || $config['allowpassword']) {
        loaducenter();
    ...
    if($config['smsAppKey'] && $config['smssecretKey'] && $config['smsFreeSignName'] && $config['smsTemplateCode']){
        ...
        if($_GET['vrcode']!=getcookie($cookiekey)){
            showmessage(lang('plugin/xigua_login', 'vcodeerror'));
        }
    }
    ...
    if(!$_GET['rb']) {  //直接注册并登录
        ...
        if ($uid>0) {
            ...
            if($_GET['mobile']){
                C::t('#wechat#common_member_wechat')->update($uid, array('mobile'=> $_GET['mobile']));
                C::t('common_member_profile')->update($uid, array('mobile'=> $_GET['mobile']));
            }
            C::t('#wechat#mobile_wechat_authcode')->update($reg_sid, array('uid' => $uid, 'status' => 1));
