...
if(
    ...
    switch ($r['target']){
        ...
        case 'target_group':
            if($r['group']){
                $openids = DB::fetch_all("SELECT w.openid,m.uid FROM %t AS w, %t AS m WHERE m.uid=w.uid AND m.groupid IN (%n) AND w.openid<>'' ORDER BY m.uid ASC LIMIT $current,$pagesize ", array($wechat_table,'common_member', $r['group']), 'uid');
            }
            $processed = $openids ? 1 : 0;
            break;
        case 'target_all':
            $openids = DB::fetch_all("SELECT w.openid,m.uid FROM %t AS w, %t AS m WHERE m.uid=w.uid AND w.openid<>'' ORDER BY m.uid ASC LIMIT $current,$pagesize ", array($wechat_table,'common_member'), 'uid');
            $processed = $openids ? 1 : 0;
            break;
        case 'target_uids':
            ...
            if($minuid && $maxuid && ($minuid<=$maxuid)){
                ...
                $openids = DB::fetch_all("SELECT w.openid,m.uid FROM %t AS w, %t AS m WHERE m.uid=w.uid AND m.uid>=$minuid AND m.uid<=$maxuid AND w.openid<>'' ORDER BY m.uid ASC LIMIT $current,$pagesize ", array($wechat_table,'common_member'), 'uid');
                $processed = $openids ? 1 : 0;
            }
            break;
        case 'by_fans':
