...
$pms = DB::fetch_all("SELECT h.*,w.openid FROM %t AS h,%t AS w WHERE h.uid=w.uid AND h.new=1 AND h.uid>0 AND w.openid<>'' ORDER BY h.id DESC LIMIT 0,1",array('home_notification', $wechat_table));
foreach ($pms as $pm) {
    if($pm){
