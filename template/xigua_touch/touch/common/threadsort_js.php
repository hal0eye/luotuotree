...
<script>
    ...
    function changeselectthreadsort(selectchoiceoptionid, optionid, type) {
        ...
        ___('select_' + forum_optionlist[soptionid]['sidentifier']).innerHTML = selectoption;
    }

    function checkoption(identifier, required, checktype, checkmaxnum, checkminnum, checkmaxlength) {
        if(checktype != 'image' && checktype != 'select' && !___('typeoption_' + identifier) || !___('check' + identifier)) {
            return true;
        }

        var ce = ___('check' + identifier);
        ce.innerHTML = '';

        if(checktype == 'select') {
            if(required != '0' && (___('typeoption_' + identifier) == null || ___('typeoption_' + identifier).value == '0')) {
                warning(ce, '{echo xtl('bitain');}');
                return false;
            } else if(required == '0' && (___('typeoption_' + identifier) == null || ___('typeoption_' + identifier).value == '0')) {
                ce.innerHTML = '<img src="' + IMGDIR + '/check_error.gif" width="16" height="16" class="vm" /> {echo xtl('xia');}';
        ...
        if(checktype == 'radio' || checktype == 'checkbox') {
            var nodes = ___('typeoption_' + identifier).parentNode.parentNode.parentNode.getElementsByTagName('INPUT');
        ...
        if(checktype == 'image') {
            var checkvalue = ___('sortaid_' + identifier).value;
        } else {
            var checkvalue = ___('typeoption_' + identifier).value;
        }
