...
var scrolltop = {
	obj : null,
	init : function(obj) {
		var caidantop = $('.caidantop');
		scrolltop.obj = obj;
		var fixed = this.isfixed();
		var tts = 600;

        obj.css('bottom', '118px');
        caidantop.css('bottom', '68px');

        obj.removeClass('bottom').hide();
        caidantop.removeClass('bottom').hide();

		obj.on('click', function() {
            $('body,html').animate({ scrollTop: 0 }, tts);
		});
	...
	open : function(pop, type, url) {
		...
		if(typeof pop == 'string') {
			...
			if(type == 'alert') {
				...
				pop = '<div class="tip"><dt>'+ pop +'</dt><dd><input class="redirect button2" type="button" value="确定" href="'+ url +'"><a href="javascript:;" class="weui-btn weui-btn_mini weui-btn_default" onclick="popup.close();">取消</a></dd></div>'
			}
		...
		$('#mask').css({'display':'block','width':'100%','height':'100%','position':'fixed','top':'0','left':'0','background':'rgba(0,0,0,.3)','z-index':'100'});
...
function ajaxget(url, id){
    $.ajax({
        type : 'GET',
        url :url+'&inajax=1&ajaxtarget='+id,
        dataType : 'xml'
    })
        .success(function(s) {
            $('#'+id).html(s.lastChild.firstChild.nodeValue);
        })
        .error(function() {
        });
}
function showWindow(target, url){
...
$(document).ready(function() {
	...
	var loadstatus = 0, curlpage = 0, currurl = window.location.href, lasthtml = '';
	if(currurl.indexOf('forum.php?mod=forumdisplay')!==-1 || currurl.indexOf('forum.php?mod=guide')!==-1){
        $(document).on('scroll', function() {
            var curtop = Math.max(document.documentElement.scrollTop, document.body.scrollTop);
            if(curtop + document.documentElement.clientHeight + 100 >= document.documentElement.scrollHeight && !loadstatus) {
                loadstatus = 1;
                curlpage = curlpage ? curlpage : parseInt($('#dumppage').val());
                if(curlpage<1 || isNaN(curlpage)){
                	return ;
				}
                if(curlpage>= parseInt($('#dumppage option:last').val())){
					return ;
                }

                var sep = currurl.indexOf('?') !== -1 ? '&' : '?';
                var nexturl = currurl.replace(/&page=\d+/, '') +sep+'page='+ (parseInt(curlpage)+1);
				...
				.success(function(s) {
					lasthtml = $(s).find('.news-list-wrapper').html();
					$('.threadlist').append(lasthtml);
                    // history.replaceState(null, "", nexturl);
                    curlpage++;
                    $('#select_a span').text($('#select_a span').text().replace(/\d+/, curlpage));
                    console.log('loaded '+ curlpage);
                    loadstatus = 0;
                    $("img.lazy").lazyload({
                        effect : "fadeIn"
                    });
	...
	}else if(currurl.indexOf('forum.php?mod=viewthread')!==-1 ){
        $(document).on('scroll', function() {
            var curtop = Math.max(document.documentElement.scrollTop, document.body.scrollTop);
            if(curtop + document.documentElement.clientHeight + 100 >= document.documentElement.scrollHeight && !loadstatus) {
                loadstatus = 1;
                ...
                $.ajax({
                    ...
                    .success(function(s) {
                    	var repHtml = '';
                        $(s).find('.postitem').each(function () {
                            repHtml+= $(this).prop("outerHTML");
                        });
                        $(repHtml).insertBefore($('#post_new'));
                        // history.replaceState(null, "", nexturl);
                        curlpage++;
                        $('#select_a span').text($('#select_a span').text().replace(/\d+/, curlpage));
                        console.log('loaded '+ curlpage);
