...
function changeselectthreadsort(selectchoiceoptionid, optionid, type) {
	...
	i_('select_' + forum_optionlist[soptionid]['sidentifier']).innerHTML = selectoption;
}

function checkoption(identifier, required, checktype, checkmaxnum, checkminnum, checkmaxlength) {
	if(checktype != 'image' && checktype != 'select' && !i_('typeoption_' + identifier) || !i_('check' + identifier)) {
		return true;
	}

	var ce = i_('check' + identifier);
	ce.innerHTML = '';

	if(checktype == 'select') {
		if(required != '0' && (i_('typeoption_' + identifier) == null || i_('typeoption_' + identifier).value == '0')) {
			warning(ce, '必填项目没有填写');
			return false;
		} else if(required == '0' && (i_('typeoption_' + identifier) == null || i_('typeoption_' + identifier).value == '0')) {
	...
	if(checktype == 'radio' || checktype == 'checkbox') {
		var nodes = i_('typeoption_' + identifier).parentNode.parentNode.parentNode.getElementsByTagName('INPUT');
	...
	if(checktype == 'image') {
		var checkvalue = i_('sortaid_' + identifier).value;
	} else {
		var checkvalue = i_('typeoption_' + identifier).value;
	}
